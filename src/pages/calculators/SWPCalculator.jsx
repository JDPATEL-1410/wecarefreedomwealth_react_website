...
const SWPCalculator = () => {
    ...
    datasets: [
      {
        ...
        data: [totalInvestment, ...yearWiseData.map(d => d.balance)],
  ...
  const lineChartOptions = {
    ...
    plugins: {
      ...
      tooltip: {
        ...
        bodyFont: { size: 13 },
        callbacks: {
          label: (context) => `Valuation: ${currencySymbols[currency]}${(context.parsed.y / 100000).toFixed(2)}L`
        }
      }
    },
    scales: {
  ...
  const doughnutData = {
    labels: ['Base Capital', 'Yield Accrual', 'Liquidity Outflow'],
    datasets: [{
      data: [results.initialInvestment, results.totalReturns, results.totalWithdrawn],
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 w-full text-center">
          ...
          >
            ...
            >
              ...
              <span className="text-orange-900 font-bold text-[10px] uppercase tracking-[0.2em]">Liquidity Architecture</span>
            </motion.div>
            ...
            <p className="text-lg md:text-xl text-slate-700 leading-relaxed max-w-2xl mx-auto font-medium">
              Engineer your systematic cash flow with actuarial precision and clinical capital preservation modeling.
            </p>
          </motion.div>
        </div>
      </section>
              ...
              <div className="p-10 space-y-10">
                ...
                <div className="bg-orange-50 p-6 rounded-[2rem] border border-orange-100">
                  <div className="flex items-start">
                    ...
                    <div>
                      <h4 className="font-black text-slate-900 text-xs uppercase tracking-widest mb-1 italic">Preservation Alpha</h4>
                      <p className="text-xs text-slate-600 font-medium leading-relaxed">
                        To sustain capital indefinitely, ensure your withdrawal rate remains below the trailing periodic yield.
                      </p>
                    </div>
                  </div>
                </div>
          ...
          <div className="lg:col-span-8 space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <ResultCard
                icon={FaPiggyBank}
                label="Base Capital"
                value={`${currencySymbols[currency]}${(results.initialInvestment / 100000).toFixed(2)}L`}
                subtext="Initial Deployment"
                color="text-slate-900"
              />
              <ResultCard
                icon={FaChartLine}
                label="Yield Accrual"
                value={`${currencySymbols[currency]}${(results.totalReturns / 100000).toFixed(2)}L`}
                subtext="Growth Generated"
                color="text-orange-600"
              />
              <ResultCard
                icon={FaTrophy}
                label="Residual Value"
                value={`${currencySymbols[currency]}${(results.finalValue / 100000).toFixed(2)}L`}
                subtext="Terminal Portfolio"
                color="text-white"
                featured
              />
            </div>
            ...
            <div className="bg-white rounded-[4rem] shadow-xl p-10 border border-slate-100">
              <div className="flex items-center justify-between mb-10">
                <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight">Financial Trajectory</h3>
              </div>
              <div className="h-96">
              ...
              <div className="bg-white rounded-[4rem] shadow-xl p-10 border border-slate-100 text-center">
                ...
                <div className="h-64 flex items-center justify-center relative">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center flex-col pointer-events-none">
                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Outflow Ratio</span>
                    <span className="text-2xl font-black text-slate-900">
                      {((results.totalWithdrawn / (results.initialInvestment + results.totalReturns)) * 100).toFixed(0)}%
                    </span>
                  </div>
                </div>
              </div>
                      ...
                      {yearWiseData.map((row) => (
                        <tr key={row.year} className="hover:bg-slate-50 transition-colors">
                          ...
                          <td className="px-6 py-4 text-xs font-bold text-slate-600 text-right">
                            {currencySymbols[currency]}{(row.balance / 100000).toFixed(2)}L
                          </td>
                        </tr>
                      ))}
                    </tbody>
            ...
            <div className="bg-slate-900 rounded-[4rem] shadow-2xl p-12 text-white relative overflow-hidden">
              ...
              <div className="grid md:grid-cols-2 gap-8">
                <div className="space-y-6">
                  <BenefitItem text="Tax Efficient Extraction" />
                  <BenefitItem text="Periodic Rebalancing" />
                  <BenefitItem text="Liquidity Optimization" />
                </div>
